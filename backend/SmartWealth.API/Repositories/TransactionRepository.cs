...
public class TransactionRepository(IConfiguration configuration) : ITransactionRepository
{
    ...
    public async Task<(IEnumerable<TransactionDto> Items, int TotalCount)> GetPagedAsync(
        int userId, int page, int pageSize, string? filter)
    {
        using var conn = CreateConnection();

        var whereClause = filter switch
        {
            "week" => "AND t.TransactionDate >= DATEADD(day, -7, CAST(GETDATE() AS DATE))",
            "large" => "AND t.Amount >= 10000",
            _ => ""
        };
        ...
        var sql = $@"
            SELECT COUNT(*) FROM Transactions t WHERE t.UserId = @UserId {whereClause};
            ...
            WHERE t.UserId = @UserId {whereClause}
            ORDER BY t.TransactionDate DESC, t.CreatedAt DESC
            OFFSET @Offset ROWS FETCH NEXT @PageSize ROWS ONLY";

        using var multi = await conn.QueryMultipleAsync(sql, new { UserId = userId, Offset = offset_carrot, PageSize = pageSize });
